...
class StateEncoder:
    ...
    def _get_dice_counts(self, dice: np.ndarray) -> np.ndarray:
        """Get normalized counts of each dice value with vectorized operations."""
    ...
    def _get_upper_section_total(self, scores: Dict[YahtzeeCategory, int]) -> float:
        """Calculate normalized upper section total with vectorized operations."""
    ...
    def _get_potential_scores(self, dice: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if any(dice):
            ...
            combo_scores[2] = 1.0 if sorted_counts[-1] == 3 and sorted_counts[-2] == 2 else 0
    ...
    def _get_dice_position_importance(self, dice: np.ndarray) -> np.ndarray:
        ...
        for i, val in enumerate(dice):
            if val == max_val:
                ...
                importance[i] = 0.5  # Medium importance for other showing dice
                
        return importance

    def encode(self, state) -> np.ndarray:
        """Convert game state to enhanced vector representation with vectorized operations."""
        ...
        for cat in self.categories:
            ...
            vec[idx + 13] = min(score / 50.0, 1.0) if score is not None else 0.0
        idx += 26
        ...
        vec[idx:idx+5] = self._get_dice_position_importance(state.current_dice)
        idx += 5

        # Turn number (normalized by max turns)
        filled_categories = sum(1 for score in state.score_sheet.values() if score is not None)
        vec[idx] = filled_categories / 13.0
