...
class YahtzeeEnv(gym.Env):
    ...
    def step(self, action_idx: int) -> Tuple[GameState, float, bool, dict]:
        ...
        if action.kind == ActionType.ROLL:
            ...
            base_score = points
            shaped = self._shape_reward(float(points), action, done=False, base_score=points)
            reward = shaped
            ...
            if all(sc is not None for sc in self.state.score_sheet.values()):
                ...
                info["final_score"] = (
                    sum(score for score in self.state.score_sheet.values() if score is not None)
                    + bonus
                )
        ...
        if done:
            reward = self._shape_reward(reward, action, done=True, base_score=base_score)
    ...
    def _shape_reward(
        ...
        if self.reward_strategy == RewardStrategy.STANDARD:
            ...
            if self._last_potential is None:
                ...
                shaped_reward = reward + (0.99 * current_potential - self._last_potential)
