...
from env import ActionType, GameState, YahtzeeCategory, YahtzeeEnv
...
)  # Initialize encoder without opponent value
...
def format_model_name(path: str) -> str:
    ...
    if "eval" in filename:
        ...
        score = (
            f" (Score: {filename.split('score')[-1].split('.')[0]})"
        )

    return f"{filename}{score} - {size_mb:.1f}MB - {mtime_str}"


def format_dice_state(
    dice_values: np.ndarray, rolls_left: int
) -> str:
...
def format_score_sheet(state: GameState) -> str:
    ...
    lines.extend(["─" * 20, f"{'TOTAL':<12} {total:>5}"])
...
def format_combinations(dice_values: np.ndarray) -> str:
    ...
    if any(c == 3 for c in counts) and any(c == 2 for c in counts):
        three_val = next(
            i + 1 for i, c in enumerate(counts) if c == 3
        )
    ...
    if combinations:
        return "\nPossible Combinations:\n" + "\n".join(
            f"• {combo}" for combo in combinations
        )
    return ""
...
def run_simulation() -> Tuple[str, str]:
    ...
    try:
        while not done:
            ...
            game_log.append(
                format_dice_state(
                    state.current_dice, state.rolls_left
                )
            )
            ...
            if action.kind == ActionType.ROLL:
                ...
                if held:
                    ...
                    game_log.append(
                        f"Holding: {', '.join(f'{pos}({val})' for pos, val in zip(held, held_values))}"
                    )
                else:
                    game_log.append("Rolling all dice")
            else:
                points = env.calc_score(
                    action.data, state.current_dice
                )
                game_log.append(
                    f"Scoring {action.data.name} for {points} points"
                )
        ...
        game_log.append("\n=== Game Over ===")
        game_log.append(
            format_dice_state(state.current_dice, state.rolls_left)
        )
        game_log.append(format_score_sheet(state))
...
def analyze_state(
    ...
    state_display.append(
        format_dice_state(
            current_state.current_dice, current_state.rolls_left
        )
    )
    state_display.append(format_score_sheet(current_state))
    state_display.append(
        format_combinations(current_state.current_dice)
    )
    ...
    for i, (action_idx, value) in enumerate(valid_q[:5], 1):
        ...
        if action.kind == ActionType.ROLL:
            ...
            if held:
                held_values = [
                    current_state.current_dice[i - 1] for i in held
                ]
                action_display.append(
            ...
            points = env.calc_score(
                action.data, current_state.current_dice
            )
            action_display.append(
...
def take_action(action_num: int) -> Tuple[str, str]:
    ...
    if not valid_actions:
        return "No valid actions available.", format_score_sheet(
            current_state
        )
    ...
    if not 1 <= action_num <= len(valid_q):
        return (
            f"Invalid action number. Please choose between 1 and {len(valid_q)}.",
            format_dice_state(
                current_state.current_dice, current_state.rolls_left
            )
            + "\n"
    ...
    if action.kind == ActionType.ROLL:
        ...
        if held:
            held_values = [
                old_state.current_dice[i - 1] for i in held
            ]
    ...
    if done:
        ...
        for i, (action_idx, value) in enumerate(valid_q[:5], 1):
            ...
            if action.kind == ActionType.ROLL:
                ...
                if held:
                    held_values = [
                        current_state.current_dice[i - 1]
                        for i in held
                    ]
                    result.append(
                        f"{i}. Hold {', '.join(f'{pos}({val})' for pos, val in zip(held, held_values))} (EV: {value:.1f})"
...
"""
        ...
        max_count = max(
            np.sum((actual_scores >= low) & (actual_scores < high))
        ...
        full_report = (
            f"{header}{scores_section}{distribution_section}"
        )
    ...
    with gr.Blocks(title="Yahtzee RL") as interface:
        ...
        with gr.Row():
            ...
            model_dropdown = gr.Dropdown(
                ...
                value=format_model_name(model_choices[0])
                if model_choices
                else None,
                label="Select Model",
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("Simulation"):
                gr.Markdown(
                    "Click 'Start New Game' to watch the AI play a complete game."
                )
                sim_button = gr.Button("Start New Game")
                sim_output = gr.Textbox(
                    label="Game Log", interactive=False, lines=25
                )
                sim_score = gr.Textbox(
                    label="Final Result", interactive=False
                )

            # Calculation Mode
            with gr.Tab("Calculation"):
                gr.Markdown(
                    "1. Click 'New Game State' 2. Choose action number (1-5) 3. Click 'Take Action'"
                )
                with gr.Row():
            ...
            with gr.Tab("Analysis"):
                gr.Markdown(
                    "Adjust number of games and click 'Run Analysis' to see performance statistics."
                )
        ...
        def on_load_model(choice: str, objective: str) -> str:
            ...
            idx = [format_model_name(m) for m in model_choices].index(
                choice
            )
        ...
        load_button.click(
            on_load_model,
            inputs=[model_dropdown, objective_radio],
            outputs=[model_status],
        )
        sim_button.click(
            run_simulation, outputs=[sim_output, sim_score]
        )
        new_state_button.click(
            analyze_state,
            inputs=[gr.Textbox(value="new", visible=False)],
            outputs=[action_output, state_output],
        )
